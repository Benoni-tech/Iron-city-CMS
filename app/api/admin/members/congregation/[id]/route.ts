...
import { getCongregationMemberById, updateCongregationMember, deleteCongregationMember } from "@/lib/firestore"
...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  const member = await getCongregationMemberById(user.tenantId, id)
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  await updateCongregationMember(user.tenantId, id, body)
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  const { id } = await params
  await deleteCongregationMember(user.tenantId, id)
  return NextResponse.json({ success: true })
}
