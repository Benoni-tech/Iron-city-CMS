  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display font-bold text-3xl text-[#1a2744]">Teens</h1>
          <p className="text-sm text-stone-400 mt-1">Children aged 11–16 · {active} active</p>
        </div>
        <Link href="/admin/members/teens/new" className="admin-btn-primary">+ Register Teen</Link>
      </div>

      {members.length === 0 ? (
        <div className="py-20 text-center border-2 border-dashed border-stone-200 rounded-xl">
          <p className="text-stone-400 italic text-sm">No Teens registered yet.</p>
          <Link href="/admin/members/teens/new" className="text-sm text-[#1a2744] underline mt-2 inline-block">Register the first Teen</Link>
        </div>
      ) : (
        <div className="bg-white border border-stone-200 rounded-xl overflow-hidden">
          ...
          {members.map((m, i) => (
            <div key={m.id} className={`grid grid-cols-[1fr_130px_100px_80px] gap-4 items-center px-5 py-3.5 hover:bg-stone-50 transition-colors ${i > 0 ? "border-t border-stone-100" : ""}`}>
              ...
              <Link href={`/admin/members/teens/${m.id}`} className="text-xs text-stone-400 hover:text-[#1a2744] transition-colors text-right">Edit →</Link>
            </div>
          ))}
        </div>
      )}
